...
export default function ProductClientPage({ product, whatsappSettings }: ProductClientPageProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // y no estamos en la sección de widget (si existe)
      const scrollPosition = window.scrollY;
      const bookingWidgetSection = document.getElementById('booking-widget-section');
      const bookingWidgetVisible = bookingWidgetSection 
        ? scrollPosition + window.innerHeight > bookingWidgetSection.offsetTop 
        : false;
      
      setShowFloatingButton(scrollPosition > 300 && !bookingWidgetVisible);
    };
  ...
  const handleShare = () => {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(window.location.href)
        ...
        .catch(() => alert('Could not share this product'));
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 pb-20 sm:py-8">
      {/* Breadcrumbs - Solo desktop */}
      <nav className="hidden sm:flex mb-4 sm:mb-8 overflow-x-auto whitespace-nowrap pb-2" aria-label="Breadcrumb">
      ...
      <div className="block sm:hidden">
        ...
        <div className="relative mb-4">
          ...
          <button 
            onClick={handleShare} 
            className="absolute top-4 right-4 bg-white/80 hover:bg-white p-2.5 rounded-full shadow-md z-10"
            aria-label="Share"
          >
        ...
        <div className="space-y-1">
          ...
          {/* FAQs Section en móvil integrado en las secciones colapsables */}
      ...
      {/* Botón flotante estilo GetYourGuide para móvil */}
      <div 
        className={`fixed bottom-0 left-0 right-0 z-50 sm:hidden transition-all duration-300 bg-white shadow-lg border-t border-gray-200 ${
