                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center">
                            {logo?.asset && (
                                <Image
                                    src={urlForImage(logo)?.width(120).height(120).url() || ''}
                                    alt={settings.title || 'Logo'}
                                    width={120}
                                    height={120}
                                    className="h-16 w-auto transition-opacity duration-200 hover:opacity-90"
                                />
                            )}
                        </div>
                        <p className="text-sm leading-relaxed">
                            {settings.footerTagline || 'Your premier boat rental service in Ibiza. Experience luxury and adventure with our fleet of modern boats, perfect for any occasion.'}
                        </p>
                        <div className="space-y-3">
                            {settings.contactEmail && (
                                <a href={`mailto:${settings.contactEmail}`} className="flex items-center text-sm hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-white rounded">
                            ...
                            {settings.contactPhoneNumber && (
                                <a href={`tel:${settings.contactPhoneNumber}`} className="flex items-center text-sm hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-white rounded">
                                    <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <ul className="space-y-3">
                            {settings.footerQuickLinks?.links?.map((link: FooterLink, index: number) => (
                                <li key={index}>
                                    <Link 
                                        href={link.url} 
                                        className="text-sm hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-white rounded"
                                    >
                                        {link.text}
                                    </Link>
                                </li>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
                    <div className="flex flex-col md:flex-row justify-between items-center space-y-3 md:space-y-0">
                        <div className="text-sm">
                            © {currentYear} {settings.footerCopyright || settings.title || 'Hang Loose Ibiza'}. All rights reserved.
                        </div>
                        {settings.footerCredits && (
                            <div className="text-sm flex items-center space-x-1">
                                <span>Made with</span>
                                <span className="text-red-500">❤️</span>
                                <span>from MDQ & IBZ</span>
                            </div>
                        )}
                    </div>
                </div>
